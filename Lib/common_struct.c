...
//	v->xbeta = (v->bs - v->cs)*0.57735; //
		v->xalpha = (v->as-0.5*v->bs-0.5*v->cs)*0.8165;
		v->xbeta = (v->bs - v->cs)*0.707; //
}
...
//    v->cs = -(v->as + v->bs);
	v->as = (v->xalpha+0.707*v->xbeta)*0.8165;
	v->bs = (-0.5*v->xalpha + 0.866*v->xbeta)*0.8165;
    v->cs = (-0.5*v->xalpha - 0.866*v->xbeta)*0.8165;
}
